...
module.exports = async (deployer, network, accounts) => {
  ...
  if (deploySwitch.DEPLOY_V2) {
    ...
    if (DefaultMtFeeRateAddress == "") {
      ...
      const defaultMtFeeRateInstance = await FeeRateModelTemplate.at(
    ...
    if (FeeRateImplAddress == "") {
      ...
      var tx = await feeRateImplInstance.init(
        multiSigAddress,
        CloneFactoryAddress,
        UserQuotaAddress
      );
    ...
    if (DefaultPermissionAddress == "") {
      ...
      const defaultPermissionInstance = await PermissionManagerTemplate.at(
    ...
    if (ERC20V2FactoryAddress == "") {
      ...
      const ERC20V2FactoryInstance = await ERC20V2Factory.at(
    ...
    if (DODOMineV3RegistryAddress == "") {
      ...
      const dodoMineV3RegistryInstance = await MineV3Registry.at(
    ...
    if (DODOMineV3ProxyAddress == "") {
      ...
      const dodoMineV3ProxyInstance = await DODOMineV3Proxy.at(
    ...
    if (network == "kovan" || network == "rinkeby" || network == "boba_test") {
      ...
      const DODOApproveProxyInstance = await DODOApproveProxy.at(
        DODOApproveProxyAddress
      );
      tx = await DODOApproveProxyInstance.init(multiSigAddress, [
        DODOV2ProxyAddress,
        DODODspProxyAddress,
        DODOCpProxyAddress,
        DODODppProxyAddress,
        DODOMineV3ProxyAddress,
        DODORouteProxyAddress,
      ]);
      logger.log("DODOApproveProxy Init tx: ", tx.tx);

      //Approve init
      const DODOApproveInstance = await DODOApprove.at(DODOApproveAddress);
      tx = await DODOApproveInstance.init(
        multiSigAddress,
      ...
      //Set FeeRateImpl
      const FeeRateModelInstance = await FeeRateModelTemplate.at(
        DefaultMtFeeRateAddress
      );
      ...
      const dodoMineV2FactoryInstance = await DODOMineV2Factory.at(
        DODOMineV2FactoryAddress
      );
      ...
      const dodoMineV3RegistryInstance = await DODOMineV3Registry.at(
        DODOMineV3RegistryAddress
      );
      var tx = await dodoMineV3RegistryInstance.addAdminList(
        DODOMineV3ProxyAddress
      );
